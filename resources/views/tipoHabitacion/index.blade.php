...
@section('titulo', 'Gestión de Tipos de Habitación')
...
<div class="container">
  <div class="header-section">
    ...
    <div class="right-buttons">
      <button type="button" class="btn yellow" id="btnExportarPdf" data-ruta-pdf="{{ route('pdf.tipo_habitacion') }}">📄 PDF</button>
      <button class="btn green" id="abrirModalCrear">Crear Tipo de Habitación</button>
    </div>
  </div>
  <!--SECCION DE FILTROS -->
  <div class="filters">
    ...
    <div class="input-group full">
      <input type="text" id="busqueda" placeholder="Busca un nombre de tipo de habitación">
    </div>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr data-id="{{ $dato['id'] }}" data-filtro-texto="{{ strtolower($dato['nombre'].' '.$dato['descripcion']) }}" data-filtro-estado="{{ $dato['estado'] }}">
        <td>{{ $dato['id'] }}</td>
        <td>{{ $dato['nombre'] }}</td>
        <td>{{ $dato['descripcion'] }}</td>
        <td>{{ $dato['cant_cama'] }}</td>
        <td>{{ $dato['precio'] }}</td>
        <td>
          @if ($dato['estado'] == 1)
        ...
        <td class="acciones">
          <button class="btn btn-edit btn-abrir-editar"
              ...
              data-cant-cama="{{ $dato['cant_cama'] }}"
              data-precio="{{ $dato['precio'] }}"
              data-estado="{{ $dato['estado'] }}">
              Editar
          </button>
          <button type="button" class="btn btn-pdf" data-ruta-pdf="{{ route('pdf.tipo_habitacion') }}" data-id="{{ $dato['id'] }}">PDF</button>
...
<div id="modalCrear" class="modal-overlay" style="display:none">
    <div class="modal-contenido">
        ...
        <form action="{{ route('crear.tipo_habitacion') }}" method="POST">
            @csrf
            ...
            <div class="campo-form">
                ...
                <input type="number" name="cant_cama" min="1" required>
            </div>
...
<div id="modalEditar" class="modal-overlay" style="display:none">
    <div class="modal-contenido">
        ...
        <form action="{{ route('editar.tipo_habitacion') }}" method="POST">
            @csrf
            ...
            <div class="campo-form">
                <label>Cantidad de Camas:</label>
                <input type="number" name="cant_cama" id="edit_cant_cama" min="1" required>
            </div>

            <div class="campo-form">
                <label>Precio:</label>
                <input type="number" name="precio" id="edit_precio" min="0" required>
            </div>
...
<div id="modalEliminar" class="modal-overlay" style="display:none">
    <div class="modal-contenido">
        ...
        <span>¿Seguro que desea eliminar este tipo de habitación?</span>

        <form action="{{ route('eliminar.tipo_habitacion') }}" method="POST">
            @csrf
...
<script>
    ...
    const editCantCama = document.getElementById('edit_cant_cama');
    ...
    document.querySelectorAll('.btn-abrir-editar').forEach(boton => {
        boton.addEventListener('click', () => {
            ...
            editCantCama.value = boton.getAttribute('data-cant-cama');
